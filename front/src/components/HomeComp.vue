...
import { planta0h, planta1h, planta1v, planta2h, planta2v, planta3v, planta3h } from '@/assets/planos/paths';

const store = useCounterStore();
const BASE_URL = 'http://localhost:8000';
const sosActive = ref(true);
const sectorInput = ref('');
const plantaInput = ref('planta1');
const dataUser = store.userData;
const alumno_id = dataUser.user.id;

function resetSector() {
    sectors3h.value.forEach((sector, i) => {
        sector.color = "white";
    });
    sectors3v.value.forEach((sector, i) => {
        sector.color = "white";
    });
}
